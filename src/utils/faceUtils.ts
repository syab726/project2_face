 ...
 * 분석 분야 한글 변환
 */
export function getFieldDisplayName(field: AnalysisField): string {
  const fieldMap: Record<AnalysisField, string> = {
    love: '연애',
    business: '사업',
    health: '건강',
    children: '자녀',
 ...
 * 분석 분야 설명 반환
 */
export function getFieldDescription(field: AnalysisField): string {
  const descriptions: Record<AnalysisField, string> = {
 ...
 * 분석 분야별 가격 반환
 */
export function getFieldPrice(field: AnalysisField): number {
 ...
 * 분석 분야별 가격 문자열 반환
 */
export function getFieldPriceString(field: AnalysisField): string {
 ...
 * Base64 이미지 데이터 검증
 */
export function validateImageData(imageData: string): { isValid: boolean; error?: string } {
  try {
    // 1. Base64 형식 검증
    ...
    // 4. 파일 크기 추정 (base64는 원본보다 약 33% 크므로)
...
export async function preprocessImage(imageData: string): Promise<string> {
  try {
    ...
    const processedBuffer = await sharp(buffer)
      .resize(800, 800, { // 최대 800x800으로 리사이즈
        fit: 'inside',
 ...
 * 분석 결과 텍스트 포맷팅
 */
export function formatAnalysisResult(result: FreeAnalysisResult | PremiumAnalysisResult): string {
  if ('fiveFeatures' in result) {
    // 무료 분석 결과 포맷팅
    const freeResult = result as FreeAnalysisResult;
    return `
## 오악 분석 결과
...
${freeResult.generalAdvice}
    ...
    // 유료 분석 결과 포맷팅
    const premiumResult = result as PremiumAnalysisResult;
 ...
 * 분석 결과를 HTML 형태로 변환
 */
export function formatAnalysisResultAsHTML(result: FreeAnalysisResult | PremiumAnalysisResult): string {
 ...
 * 분석 결과 요약 생성
 */
export function generateAnalysisSummary(result: FreeAnalysisResult | PremiumAnalysisResult): string {
  if ('fiveFeatures' in result) {
    const freeResult = result as FreeAnalysisResult;
 ...
 * 분석 가능 여부 확인
 */
export function canAnalyze(imageData: string): { canAnalyze: boolean; reason?: string } {
  // 이미지 데이터 검증
...
/**
 * 이미지 파일 크기 계산 (base64에서)
 */
export function calculateImageSize(base64Data: string): number {
  // Base64는 원본보다 약 33% 크므로 실제 크기 계산
 ...
 * 분석 통계 생성
 */
export function generateAnalysisStats(results: (FreeAnalysisResult | PremiumAnalysisResult)[]): {
  ...
  const stats = {
    ...
    freeAnalyses: 0,
    premiumAnalyses: 0,
    fieldDistribution: {} as Record<string, number>
  };
  ...
  results.forEach(result => {
    if ('fiveFeatures' in result) {
      ...
      const field = (result as PremiumAnalysisResult).selectedField;
