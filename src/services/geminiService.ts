...
export class GeminiService {
   ...
   * 무료 분석 - 오악 분석만
   */
  async analyzeFaceFree(imageData: string): Promise<GeminiAnalysisResponse> {
    try {
      const model = this.genAI.getGenerativeModel({ model: this.freeModel });
   ...
   * 유료 분석 - 전문 분야별 분석
   */
  async analyzeFacePremium(imageData: string, selectedField: AnalysisField): Promise<GeminiAnalysisResponse> {
    try {
   ...
   * Gemini API 에러 처리
   */
  private handleGeminiError(error: any): FaceAnalysisError {
