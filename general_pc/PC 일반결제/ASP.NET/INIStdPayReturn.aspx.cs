...
public partial class INIStdPayReturn : System.Web.UI.Page
{
    ...
    private void StartINIStdReturn()
    {
		...
		if (("0000".Equals(parameters["resultCode"])) && (parameters.Get("authUrl").Equals(ConfigurationManager.AppSettings[parameters.Get("idc_name")])))
			
		    // 인증성공 resultCode=0000 확인
		    // IDC센터 확인 [idc_name=fc,ks,stg]	
		    // idc_name 으로 수신 받은 값 기준 web.config(properties)에 설정된 승인URL과 authURL 이 같은지 비교
		...
		{ 
            ...
            String authUrl = parameters.Get("authUrl");				// 승인요청 API url(상단 idc_name 비교 필수)
            ...
            String signParam2 = "authToken=" + @authToken + "&signKey=" + signKey + "&timestamp=" + timestamp;
